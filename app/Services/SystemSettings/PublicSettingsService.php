...
class PublicSettingsService
{
    ...
    public function shareableSettings(): array
    {
        ...
        return [
            ...
            'general.default_dashboard_route' => $settings['general']['default_dashboard_route'] ?? 'dashboard',
            ...
            'localization.first_day_of_week' => $settings['localization']['first_day_of_week'] ?? '1',
            'localization.number_format' => $settings['localization']['number_format'] ?? '1,234.56',
            'localization.organization_name_display' => $settings['localization']['organization_name_display'] ?? 'english',
            'localization.employee_name_display' => $settings['localization']['employee_name_display'] ?? 'full_name',
            /*
             * HandleInertiaRequests reads this from the shared payload to pick
             * the calendar system for every request. It was never emitted here,
             * so the lookup always fell through to 'locale_based' and the
             * gregorian_only / ethiopian_only choices could not take effect.
             */
            'localization.calendar_system_mode' => $settings['localization']['calendar_system_mode'] ?? 'locale_based',
            'appearance.default_theme' => $settings['appearance']['default_theme'] ?? 'system',
            'appearance.primary_color' => $settings['appearance']['primary_color'] ?? '#122170',
            'appearance.secondary_color' => $settings['appearance']['secondary_color'] ?? '#1D3084',
            'appearance.accent_color' => $settings['appearance']['accent_color'] ?? '#D12908',
            'appearance.sidebar_color' => $settings['appearance']['sidebar_color'] ?? '#FFFFFF',
            ...
            'appearance.dashboard_layout' => $settings['appearance']['dashboard_layout'] ?? 'executive',
            'appearance.dashboard_refresh_seconds' => $settings['appearance']['dashboard_refresh_seconds'] ?? 60,
            'appearance.logo_position' => $settings['appearance']['logo_position'] ?? 'start',
            'appearance.show_breadcrumbs' => $settings['appearance']['show_breadcrumbs'] ?? true,
            'appearance.sticky_table_headers' => $settings['appearance']['sticky_table_headers'] ?? true,
            ...
            // Public website presentation (Public Site Management → Settings).
            'public_site.enabled' => $settings['public_site']['enabled'] ?? true,
            'public_site.maintenance_notice_en' => $settings['public_site']['maintenance_notice_en'] ?? '',
            'public_site.maintenance_notice_am' => $settings['public_site']['maintenance_notice_am'] ?? '',
            'public_site.announcements_page_size' => $settings['public_site']['announcements_page_size'] ?? 10,
            'public_site.services_page_size' => $settings['public_site']['services_page_size'] ?? 12,
            'public_site.office_hours_en' => $settings['public_site']['office_hours_en'] ?? '',
            'public_site.office_hours_am' => $settings['public_site']['office_hours_am'] ?? '',
            'public_site.office_location_en' => $settings['public_site']['office_location_en'] ?? '',
            'public_site.office_location_am' => $settings['public_site']['office_location_am'] ?? '',
            'public_site.footer_description_en' => $settings['public_site']['footer_description_en'] ?? '',
            'public_site.footer_description_am' => $settings['public_site']['footer_description_am'] ?? '',
            'public_site.copyright_text_en' => $settings['public_site']['copyright_text_en'] ?? '',
            'public_site.copyright_text_am' => $settings['public_site']['copyright_text_am'] ?? '',
            ...
            // ID Cards
            'id_cards.template' => $settings['id_cards']['template'] ?? 'classic',
            ...
            'id_cards.show_photo' => $settings['id_cards']['show_photo'] ?? true,
            'id_cards.show_full_name_en' => $settings['id_cards']['show_full_name_en'] ?? true,
            'id_cards.show_full_name_am' => $settings['id_cards']['show_full_name_am'] ?? true,
            'id_cards.show_employee_number' => $settings['id_cards']['show_employee_number'] ?? true,
            'id_cards.show_card_number' => $settings['id_cards']['show_card_number'] ?? true,
            'id_cards.show_organization' => $settings['id_cards']['show_organization'] ?? true,
            'id_cards.show_organization_unit' => $settings['id_cards']['show_organization_unit'] ?? true,
            'id_cards.show_position' => $settings['id_cards']['show_position'] ?? true,
            'id_cards.show_job_grade' => $settings['id_cards']['show_job_grade'] ?? true,
            'id_cards.show_employment_status' => $settings['id_cards']['show_employment_status'] ?? true,
            'id_cards.show_issue_date' => $settings['id_cards']['show_issue_date'] ?? true,
            'id_cards.show_expiry_date' => $settings['id_cards']['show_expiry_date'] ?? true,
            'id_cards.show_signature' => $settings['id_cards']['show_signature'] ?? false,
            'id_cards.show_qr' => $settings['id_cards']['show_qr'] ?? true,
            'id_cards.show_return_notice' => $settings['id_cards']['show_return_notice'] ?? true,
            'id_cards.show_emergency_contact' => $settings['id_cards']['show_emergency_contact'] ?? true,
            'id_cards.verification_url' => $settings['id_cards']['verification_url'] ?? null,
            ...
            'id_cards.back_notice_am' => $settings['id_cards']['back_notice_am'] ?? 'ካርዱ የሚወዳደርበት ቦታ ካገኙ ወደ ቅርብ ባለስልጣን ይመልሱ።',
            'id_cards.back_notice_en' => $settings['id_cards']['back_notice_en'] ?? 'If found, please return to the issuing bureau.',

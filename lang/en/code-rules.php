<?php

declare(strict_types=1);

return [
    'created' => 'Code rule created successfully.',
    'updated' => 'Code rule updated successfully.',
    'archived' => 'Code rule deleted successfully.',
    'restored' => 'Code rule restored successfully.',
    'invalid_format' => 'Invalid code rule format.',
    'format_must_contain_sequence' => 'Format must contain {SEQUENCE}, {SEQUENCE_PADDED}, {RAND_6}, or {RAND_8}.',
    'duplicate_active_rule' => 'A duplicate active code rule already exists for this entity and scope.',
    'manual_override_not_allowed' => 'Manual code override is not allowed for this rule.',
    'generated_code_exists' => 'The generated code already exists.',
    'random_code_duplicate' => 'Random code duplicate, please try again.',
    'unable_to_generate_unique_code' => 'Unable to generate unique code.',
    'no_active_rule' => 'No active code rule is configured for this entity.',
    'owner_organization_required' => 'An owner organization is required to generate a position code.',
    'owner_organization_inactive' => 'The owner organization is not active, so no new position code can be generated for it.',
    'host_organization_inactive' => 'The host organization is not active, so no new position code can be generated for it.',
    'scope_type_organization' => 'Organization',
    'scope_type_organization_type' => 'Organization Type',
    'scope_type_organization_unit_type' => 'Organization Unit Type',
    'scope_type_service_type' => 'Service Type',
    // Format token UI
    'format_tokens' => 'Format Tokens',
    'insert_token' => 'Insert Token',
    'copy_token' => 'Copy Token',
    'search_tokens' => 'Search Tokens',
    'token_categories' => 'Token Categories',
    'category_core' => 'Core Tokens',
    'category_date_time' => 'Date and Time Tokens',
    'category_organization' => 'Organization Tokens',
    'category_employee' => 'Employee Tokens',
    'category_position' => 'Position Tokens',
    'category_service' => 'Service Tokens',
    'category_location' => 'Location Tokens',
    'category_workflow' => 'Workflow Tokens',
    'category_custom' => 'Custom Tokens',
    'used_tokens' => 'Used Tokens',
    'unknown_token' => 'Unknown Token',
    'missing_context' => 'Missing Context',
    'token_requires_context' => 'This token requires additional context',
    'preview_requires_context' => 'Preview requires context',
    'format_examples' => 'Format Examples',
    'token_inserted' => 'Token inserted',
    'invalid_token_in_format' => 'Unknown token :token in format.',
    'click_token_to_insert' => 'Click a token to insert it into the format',
    // Validation
    'validation_format_required' => 'The format field is required.',
    'validation_format_unknown_token' => 'Unknown token :token in format.',
    'validation_format_needs_sequence' => 'Format must contain {SEQUENCE}, {SEQUENCE_PADDED}, {RAND_6}, or {RAND_8}.',
    'validation_sequence_length_required' => 'Sequence length is required when using {SEQUENCE_PADDED}.',
    // Sequence scope
    'missing_sequence_scope_context' => 'Missing sequence scope context for token: :token',
    'sequence_reset_success' => 'Sequence reset successfully.',
    'sequence_backfill_complete' => 'Sequence backfill completed.',
    'custom_tokens_required' => 'At least one scope token is required when using the Custom Tokens strategy.',
    'custom_tokens_cannot_include_sequence' => 'SEQUENCE, SEQUENCE_PADDED, RAND_6, and RAND_8 cannot be used as scope tokens.',
    'custom_tokens_must_exist_in_format' => 'Scope token :token must appear in the format string.',
];
